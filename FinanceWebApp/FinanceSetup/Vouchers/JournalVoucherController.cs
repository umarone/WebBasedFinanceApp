...
namespace MudasirRehmanAlp.FinanceSetup.Vouchers
{
    public class JournalVoucherController : Controller
    {
        ...
        public ActionResult Index(int page = 1, int pageSize = 15)
        {
            ...
            listobj = db.VouchersHeads.Where(a => a.VoucherType == CommonEnums.VoucherType.JV && a.IsDeleted == false && a.OrganizationID == OrganizationID && a.BranchId == BranchId).OrderByDescending(a => a.VoucherID).ToList();
            PagedList<VouchersHead> model = new PagedList<VouchersHead>(listobj, page, pageSize);
            return View(model);
        }

        // GET: JournalVoucher/Details/5
        ...
        // GET: JournalVoucher/Create
        public ActionResult Create()
        {
            long OrganizationID = Convert.ToInt64(Session["OrganizationID"]);
            long BranchId = Convert.ToInt64(Session["BranchId"]);
            VouchersHead obj = new VouchersHead();
            obj.VoucherCode = dALCode.AutoGenerateVouchersJVCode(OrganizationID, OrganizationID);
        ...
        public ActionResult Create(string ObjMasterItem, string ObjChilds)
        {
            ...
            var getmessage = vouchersDALObj.AddJournalVoucher(ObjMasterItem, ObjChilds, UserID);
            if (getmessage != "")
            {
            ...
            else
            {
                ...
                TempData["ErrorMessage"] = "Journal Voucher has been saved successfully";
                return Json("OK", JsonRequestBehavior.AllowGet);
            }
        }

        // GET: JournalVoucher/Edit/5
        ...
        {
            long UserID = Convert.ToInt64(Session["UserID"]);
            var getmessage = vouchersDALObj.UpdateJournalVoucher(ObjMasterItem, ObjChilds, UserID);
            if (getmessage != "")
            {
                TempData["Validation"] = false;
                TempData["ErrorMessage"] = getmessage;
            ...
            {
                TempData["Validation"] = true;
                TempData["ErrorMessage"] = "Journal Voucher has been updated successfully";
                return Json("OK", JsonRequestBehavior.AllowGet);
            }
        }

        // GET: JournalVoucher/Delete/5
        public ActionResult Delete(long? id)
        {
            ...
            var getmessage = vouchersDALObj.DeleteJournalVoucher(vouchersHead, UserID);
            if (getmessage != "")
            {
                TempData["Validation"] = false;
                TempData["ErrorMessage"] = getmessage;
            ...
            {
                TempData["Validation"] = true;
                TempData["ErrorMessage"] = "Journal Voucher has been deleted successfully";
                return RedirectToAction("Index");
            }
        }

        // POST: JournalVoucher/Delete/5
        ...
        public ActionResult Search(string searchModel)
        {
            ...
            _listObject = db.VouchersHeads.Where(a => a.IsActive == true && a.IsDeleted == false && a.OrganizationID == OrganizationID && a.VoucherType == CommonEnums.VoucherType.JV).ToList();
