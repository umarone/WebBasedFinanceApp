...
namespace MudasirRehmanAlp.FinanceSetup.Vouchers
{
    public class CashPaymentVoucherController : Controller
    {
        ...
        public ActionResult Index(int page = 1, int pageSize = 15)
        {
            ...
            listobj = db.VouchersHeads.Where(a => a.VoucherType == CommonEnums.VoucherType.CPV && a.IsDeleted == false && a.OrganizationID == OrganizationID && a.BranchId == BranchId).OrderByDescending(a => a.VoucherID).ToList();
            PagedList<VouchersHead> model = new PagedList<VouchersHead>(listobj, page, pageSize);
            return View(model);
        }

        // GET: CashPaymentVoucher/Details/5
        ...
        // GET: CashPaymentVoucher/Create
        public ActionResult Create()
        {
            long OrganizationID = Convert.ToInt64(Session["OrganizationID"]);
            long BranchId = Convert.ToInt64(Session["BranchId"]);
            VouchersHead obj = new VouchersHead();
            obj.VoucherCode = dALCode.AutoGenerateVouchersCPVCode(OrganizationID, BranchId);
        ...
        // POST: CashPaymentVoucher/Create
        ...
        public ActionResult Create(string ObjMasterItem, string ObjChilds)
        {
            ...
            var getmessage = vouchersDALObj.AddCashPaymentVoucher(ObjMasterItem, ObjChilds, UserID);
            if (getmessage != "")
            {
            ...
            else
            {
                ...
                TempData["ErrorMessage"] = "Cash Payment Voucher has been saved successfully";
                return Json("OK", JsonRequestBehavior.AllowGet);
            }
        }

        // GET: CashPaymentVoucher/Edit/5
        ...
        // POST: CashPaymentVoucher/Edit/5
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        public ActionResult Edit(string ObjMasterItem, string ObjChilds)
        {
            ...
            long UserID = Convert.ToInt64(Session["UserID"]);
            var getmessage = vouchersDALObj.UpdateCashPaymentVoucher(ObjMasterItem, ObjChilds, UserID);
            if (getmessage != "")
            {
                TempData["Validation"] = false;
                TempData["ErrorMessage"] = getmessage;
            ...
            {
                TempData["Validation"] = true;
                TempData["ErrorMessage"] = "Cash Payment Voucher has been updated successfully";
                return Json("OK", JsonRequestBehavior.AllowGet);
            }
        }

        // GET: CashPaymentVoucher/Delete/5
        public ActionResult Delete(long? id)
        {
            ...
            var getmessage = vouchersDALObj.DeleteCashPaymentVoucher(vouchersHead, UserID);
            if (getmessage != "")
            {
                TempData["Validation"] = false;
                TempData["ErrorMessage"] = getmessage;
            ...
            {
                TempData["Validation"] = true;
                TempData["ErrorMessage"] = "Cash Payment Voucher has been deleted successfully";
                return RedirectToAction("Index");
            }
        }

        // POST: CashPaymentVoucher/Delete/5
        [HttpPost, ActionName("Delete")]
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmed(long id)
        {
            VouchersHead vouchersHead = db.VouchersHeads.Find(id);
            db.VouchersHeads.Remove(vouchersHead);
            db.SaveChanges();
            return RedirectToAction("Index");
        }
        ...
        public ActionResult Search(string searchModel)
        {
            ...
            _listObject = db.VouchersHeads.Where(a => a.IsActive == true && a.IsDeleted == false && a.OrganizationID == OrganizationID && a.VoucherType == CommonEnums.VoucherType.CPV).ToList();
