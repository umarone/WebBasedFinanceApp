    ...
    public class IncomeStatementReportController : Controller
    {
        ...
        public ActionResult IncomeStatementSearch(string searchModel)
        {
            try
            {
                ...
                List<ProcIncomeStatement> _listObject = new List<ProcIncomeStatement>();

                if (_searchObject.StartDate != null && _searchObject.EndDate != null && FinancialBookYearId !=0)
                {
                    _listObject = db.ProcIncomeStatements(OrganizationID, FinancialBookYearId, _searchObject.StartDate, _searchObject.EndDate);
                }
        ...
        public ActionResult IncomeStatementDownloadExcel(DownloadFilesViewModel viewModel)
        {
            try
            {
                ...
                using (var package = excelPackage.IncomeStatement_CreateExcelPackage(_listObject, BrancheObj))
                {
                    excelReportBytes = package.GetAsByteArray();
                }
                return File(excelReportBytes, XlsxContentType, "IncomeStatement_" + DateTime.Now.Ticks + ".xlsx");
